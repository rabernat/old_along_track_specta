...
The altimeter products were produced by Ssalto/Duacs and distributed by Aviso, with support from Cnes (\url{http://www.aviso.oceanobs.com/duacs/}). For this study we used the delayed-time, merged sea-level anomaly fields and the associated geostrophic velocities. In the equatorial band ($\pm 5^\circ$) the method of \citet{Lagerloef1999} is applied to derive velocities. This method, based on the ``equatorial geostrophic'' vorticity balance, has been validated with {\em in situ} current meters and allows us to obtain velocity estimates in this region. Nevertheless, we must maintain some skepticism of the results in the equatorial band.
